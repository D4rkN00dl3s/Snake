...
void push_front(pair<int, int> pos) { head = mod(head - 1); snakeBuffer[head] = pos; snakeSize++; snakeBody.insert(pos); }
void pop_back() { tail = mod(tail - 1); snakeBody.erase(snakeBuffer[tail]); snakeSize--; }
...
void settingsMenu() {
    while (true) {
        ...
        if (ch == '1') {
            ...
            cout << "Enter speed (50-500 ms): ";
            cout.flush();
            while ((value = getInput() - '0'), (value < 5 || value > 50)) usleep(10000);
            snakeSpeed = value * 10000;
            moveCursorTo(rows / 2 + 1, cols / 2 - 10); cout << "Speed updated!"; cout.flush(); usleep(500000);
            clearTerminal();
...
void UpdateSnake(int top, int left) {
    ...
    bool ate = false;
    for (const auto& food : foodPositions) {
        if (newRow == food.first && newCol == food.second) {
            score++;
            ate = true;
            break;
        }
